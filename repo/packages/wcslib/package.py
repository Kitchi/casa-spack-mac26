...
class Wcslib(AutotoolsPackage):
    ...
    def setup_build_environment(self, env):
        # macOS install_name_tool needs enough header padding to rewrite
        # the dylib install name to an absolute path during Spack relocation.
        if self.spec.satisfies("platform=darwin"):
            env.append_flags("LDFLAGS", "-Wl,-headerpad_max_install_names")
